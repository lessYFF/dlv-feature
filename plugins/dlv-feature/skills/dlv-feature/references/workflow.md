...
```text
需求复核 → PRD ↔ 原型确认（按需）→ 架构收敛预审 → 技术方案 → Code Spec → Sealed Proof Contract → Code → Verification Run → 确定性收口
```
...
| Code Spec | 技术方案 fresh | 决策映射到文件、符号、规则、有界批次、`ENV/PO/ASRT` 并获批准和 seal |
...
| Code Spec 规则、批次、写入范围或 `ENV/PO/ASRT` | Proof Contract seal、Code、Verification |
...
每次恢复先运行 `invalidate_downstream.py`；用户修正已批准行为时用 `--from-stage` 指明最早变化层。只接受 `schema_version=7`。v6 必须用 `upgrade_v6_to_v7.py` 保守迁移；旧自由文本 PO、手写 Verification 和完成裁决一律不沿用。
